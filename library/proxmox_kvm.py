...
def create_vm(module, proxmox, vmid, newid, node, name, memory, cpu, cores, sockets, timeout, update, **kwargs):
    ...
    if update:
        ...
        if 'net' in kwargs:
            try:
                get_vminfo(module, proxmox, node, vmid, net=module.params['net'])
            except:
                pass
            if results:
                pat = re.compile('.*virtio(,|$)')
                for n in results['mac']:
                    if re.match(pat,kwargs['net'][n]): # NIC defined without MAC
                        kwargs['net'][n] = kwargs['net'][n].replace('virtio', 'virtio=%s' % results['mac'][n])
